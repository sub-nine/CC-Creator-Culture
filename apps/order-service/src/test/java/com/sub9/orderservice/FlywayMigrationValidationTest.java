...
@DisplayName("주문 서비스 Flyway 마이그레이션 검증")
class FlywayMigrationValidationTest {

    @Container
    static final PostgreSQLContainer POSTGRES = new PostgreSQLContainer("postgres:17-alpine")
            .withDatabaseName("order_service_flyway_test")
            .withUsername("test")
            .withPassword("test");
    ...
    void when_flyway_migration_is_applied_entity_tables_exist_in_public_schema() {
        ...
        assertThat(tables).containsExactly(
                "p_carts",
                "p_coupons",
                "p_order_cart_cleanup_tasks",
                "p_order_command_requests",
                "p_order_items",
                "p_orders",
                "p_payment_cancellations",
                "p_payments",
                "p_user_coupons");
    }
}
