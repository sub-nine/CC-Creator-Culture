...
public class ProductImageUploadedKafkaPublisher implements ProductImageUploadedPublisher {
  ...
  @Override
  public CompletableFuture<Void> publish(ProductImageUploadedEvent event) {
    try {
      String payload = jsonMapper.writeValueAsString(event);

      return kafkaTemplate
          .send(KafkaTopics.PRODUCT_IMAGE_UPLOADED, payload)
          .thenAccept(result -> log.info("[KAFKA] 메시지 발행 성공 topic = {}, eventId = {}", KafkaTopics.PRODUCT_IMAGE_UPLOADED, event.eventId()))
          .whenComplete(
              (result, exception) -> {
                if (exception != null) {
