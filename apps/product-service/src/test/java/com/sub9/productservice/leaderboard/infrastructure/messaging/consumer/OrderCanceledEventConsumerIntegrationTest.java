...
@SpringBootTest
@DisplayName("OrderCanceledEventConsumer - 통합 테스트")
class OrderCanceledEventConsumerIntegrationTest extends AbstractKafkaIntegrationTest {
    ...
    void setUp() {
        ...
        ContainerTestUtils.waitForAssignment(
                findContainerForTopic(KafkaTopics.ORDER_CANCELED), embeddedKafka.getPartitionsPerTopic());
    }
    ...
    @Test
    @DisplayName("주문 취소 이벤트를 수신하면 상품이 속한 카테고리/해시태그의 실시간 점수를 차감한다")
    void consume_decrementsCategoryAndHashtagScores() throws Exception {
        // Given - 이미 ORDER_PAID로 반영돼있던 점수가 있는 상황을 흉내내기 위해 미리 점수를 올려둔다
        ...
        stringRedisTemplate.opsForZSet().incrementScore(
                LeaderboardRedisKey.current(LeaderboardType.CATEGORY), category.getId().toString(), 6.0);
        stringRedisTemplate.opsForZSet().incrementScore(
                LeaderboardRedisKey.current(LeaderboardType.HASHTAG), hashtag.getId().toString(), 6.0);

        UUID orderId = UUID.randomUUID();
        OrderCanceledEvent event = new OrderCanceledEvent(
                orderId, List.of(new OrderCanceledEvent.ProductQuantity(productId, 4L)));
        ...
        await().atMost(Duration.ofSeconds(20)).untilAsserted(() -> {
            ...
            assertThat(categoryScore).isEqualTo(0.0);
            assertThat(hashtagScore).isEqualTo(0.0);
        });
    }
