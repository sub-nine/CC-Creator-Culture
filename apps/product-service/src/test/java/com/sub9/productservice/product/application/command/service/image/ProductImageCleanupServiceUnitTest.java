...
class ProductImageCleanupServiceUnitTest {
  ...
  void deleteExpiredImages_fails_without_removing_failed_image() {
    ...
    given(imageRepository.findExpiredImages(now.minusSeconds(7 * 86400)))
    ...
    // then
    verify(imageRepository, never()).hardDeleteById(failed.getId());
    verify(imageRepository).hardDeleteById(next.getId());
  }
}
