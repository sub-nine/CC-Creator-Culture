...
/**
 * 시나리오 설명: 마이페이지에서 내가 보유한 쿠폰함을 조회하는 흐름
 * 엔드포인트: GET /api/v1/user-coupons
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 30
 * 목표 TPS: 20 req/s
 * 목표 P95: 300ms
...
export function setup() {
  ...
  const email = `k6-test-usercoupons-${unique}@example.com`;
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/user-coupons`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
