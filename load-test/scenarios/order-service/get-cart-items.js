...
export function setup() {
  ...
  const skuId = createProductWithSku(config.baseUrl, creatorToken, unique, 'k6 장바구니 조회 테스트 상품');

  const customerUnique = `${Date.now()}`;
  const email = `k6-test-cart-${customerUnique}@example.com`;
  ...
  const signupRes = http.post(
    ...
    JSON.stringify({
      email,
      password,
      nickname: `k6cart${customerUnique}`,
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/cart/items`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
