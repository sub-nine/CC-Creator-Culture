...
/**
 * 시나리오 설명: 마이페이지에서 내 주문 내역을 조회하는 흐름
...
export function setup() {
  ...
  const creatorToken = signupApprovedCreator(config.baseUrl, 'listorders', unique);
  const skuId = createProductWithSku(config.baseUrl, creatorToken, unique, 'k6 주문 목록 테스트 상품');

  const customerUnique = `${Date.now()}`;
  const email = `k6-test-listorders-${customerUnique}@example.com`;
  ...
  const signupRes = http.post(
    ...
    JSON.stringify({
      email,
      password,
      nickname: `k6listorders${customerUnique}`,
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/orders`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
