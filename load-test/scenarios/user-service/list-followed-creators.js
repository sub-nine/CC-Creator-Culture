...
/**
 * 시나리오 설명: 내 팔로우 목록 페이지 진입 시 호출되는 팔로우한 크리에이터 목록 조회
 * 엔드포인트: GET /api/v1/follows (CUSTOMER 권한 필요)
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 30
 * 목표 TPS: 20 req/s
 * 목표 P95: 300ms
...
export function setup() {
  ...
  const email = `k6-test-follows-${unique}@example.com`;
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/follows`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
