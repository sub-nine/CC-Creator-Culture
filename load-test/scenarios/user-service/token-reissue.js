import http from 'k6/http';
import { sleep } from 'k6';
import config from '../../config/index.js';
import { checkStatus } from '../../lib/checks.js';

/**
 * 시나리오 설명: 액세스 토큰 만료 시 리프레시 토큰으로 재발급받는 흐름 (재발급은 리프레시 토큰을 소모하지 않아 반복 호출이 안전함)
 * 엔드포인트: POST /api/v1/auth/reissue (공개 엔드포인트)
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 40
 * 목표 TPS: 27 req/s
 * 목표 P95: 300ms
 * 허용 에러율: 1% 미만
 * 프로파일 선정 이유: 반복마다 sleep(1)이 있어 VU당 최대 처리량이 초당 1건이라, 최대 VU 40 기준 램프업/다운 구간까지 포함한 전체 평균 기준 실측 상한(~32 req/s)보다 여유 있게 목표를 잡음
 */
export const options = {
  stages: [
    { duration: '30s', target: 40 },
    { duration: '1m', target: 40 },
    { duration: '10s', target: 0 },
  ],
  thresholds: {
    http_req_duration: ['p(95)<300'],
    http_req_failed: ['rate<0.01'],
    http_reqs: ['rate>=27'],
  },
};

export function setup() {
  const unique = `${Date.now()}`;
  const email = `k6-test-reissue-${unique}@example.com`;
  const password = 'Passw0rd!';

  const signupRes = http.post(
    `${config.baseUrl}/api/v1/auth/signup/customer`,
    JSON.stringify({
      email,
      password,
      nickname: `k6reissue${unique}`,
      phone: `010-${unique.slice(-8, -4)}-${unique.slice(-4)}`,
      address: '서울시 강남구',
    }),
    { headers: { 'Content-Type': 'application/json' } },
  );
  if (signupRes.status !== 201) {
    throw new Error(`setup 실패 - 회원가입 status=${signupRes.status} body=${signupRes.body}`);
  }

  const loginRes = http.post(
    `${config.baseUrl}/api/v1/auth/login`,
    JSON.stringify({ email, password }),
    { headers: { 'Content-Type': 'application/json' } },
  );
  if (loginRes.status !== 200) {
    throw new Error(`setup 실패 - 로그인 status=${loginRes.status} body=${loginRes.body}`);
  }

  return { refreshToken: loginRes.json().data.refreshToken };
}

export default function (data) {
  const res = http.post(
    `${config.baseUrl}/api/v1/auth/reissue`,
    JSON.stringify({ refreshToken: data.refreshToken }),
    { headers: { 'Content-Type': 'application/json' } },
  );
  checkStatus(res, 200);
  sleep(1);
}
