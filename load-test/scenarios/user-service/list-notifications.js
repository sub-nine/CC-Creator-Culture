...
/**
 * 시나리오 설명: 알림함 진입 시 가장 먼저 호출되는 내 알림 목록 조회
 * 엔드포인트: GET /api/v1/notifications/ (인증 필요)
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 30
 * 목표 TPS: 20 req/s
 * 목표 P95: 300ms
...
export function setup() {
  ...
  const email = `k6-test-notifications-${unique}@example.com`;
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/notifications/`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
