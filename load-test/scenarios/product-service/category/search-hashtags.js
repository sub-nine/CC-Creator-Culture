...
/**
 * 시나리오 설명: 상품 등록/검색 화면에서 해시태그를 키워드로 찾아보는 흐름
 * 엔드포인트: GET /api/v1/hashtags (게이트웨이 정책상 로그인 필요)
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 30
 * 목표 TPS: 20 req/s
 * 목표 P95: 250ms
...
export function setup() {
  ...
  const email = `k6-test-hashtags-${unique}@example.com`;
...
export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/hashtags`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
