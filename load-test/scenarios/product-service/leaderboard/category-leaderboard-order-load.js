...
export function setup() {
  ...
  const email = `k6-test-catleaderboardorder-${unique}@example.com`;
  ...
  const categoryName = `k6clbo${unique.slice(-6)}`;
  const categoryRes = http.post(
    `${config.baseUrl}/api/v1/admin/categories`,
    JSON.stringify({ name: categoryName, description: 'k6 카테고리 리더보드(주문) 부하 테스트용 시드 카테고리' }),
  ...
  const creatorToken = signupApprovedCreator(config.baseUrl, 'catlboseed', unique);
  ...
  skuIds.forEach((skuId, i) => {
    ...
    orderHeaders.headers['Idempotency-Key'] = `k6-test-catlbo-${unique}-${i}`;
  ...
  for (let attempt = 0; attempt < POLL_MAX_ATTEMPTS; attempt++) {
    ...
    if (match && match.score >= EXPECTED_CATEGORY_SCORE - 0.01) {
      ready = true;
      break;
    }
  }
