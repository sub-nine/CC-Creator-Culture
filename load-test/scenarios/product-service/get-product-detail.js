 ...
 * 엔드포인트: GET /api/v1/products/{productId} (게이트웨이 정책상 로그인 필요)
 * 테스트 유형: 부하 테스트 (load)
 * 최대 VUser: 40
 * 목표 TPS: 27 req/s
 * 목표 P95: 300ms
...
export function setup() {
  ...
  const email = `k6-test-productdetail-${unique}@example.com`;
  ...
  const listRes = http.get(`${config.baseUrl}/api/v1/products`, authHeaders(token));
  const products = listRes.json().data.content;
  if (!products || products.length === 0) {
    throw new Error('setup 실패 - 조회할 상품 데이터가 없음 (최소 1개 상품 필요)');
  }

  return { token, productId: products[0].productId };
}

export default function (data) {
  const res = http.get(`${config.baseUrl}/api/v1/products/${data.productId}`, authHeaders(data.token));
  checkStatus(res, 200);
  sleep(1);
}
